...
hexwall <- function(path, sticker_row_size = 16, sticker_width = 500, remove_small = TRUE, total_stickers = NULL, remove_size = TRUE,
  ...
  if(is.null(coords)){
    ...
    extra_stickers <- sum(row_lens) - length(stickers)  # Calculate excess stickers
    
    if (extra_stickers > 0) {
      # Reduce row sizes to fit exactly the number of stickers
      for (i in length(row_lens):1) {
        row_lens[i] <- max(0, row_lens[i] - 1)  # Ensure row length is non-negative
        if (sum(row_lens) <= length(stickers)) break
      }
    }
    row_lens[length(row_lens)] <- row_lens[length(row_lens)] - (sum(row_lens) - length(stickers))
    sticker_rows <- map2(
      row_lens,
      cumsum(row_lens),
